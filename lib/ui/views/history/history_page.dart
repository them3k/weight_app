  ...
  Widget build(BuildContext context) {
    ...
    return BaseWidget(
        ...
        builder: (context, model, child) {
          ...
          : Column(
            children: [
              HistoryAppBar(title: 'History', isItemSelected: model.isItemsSelected,),
              SizedBox(
                height: listHeight,
                child: ListView.builder(
                    ...
                    itemBuilder: (context, position) {
                      return WeightItem(
                        item: WeightPresentation.toPresentation(model.weights[position]),
                        onLongItemPress: (int index) => {
                          setState(() {
                        context.read<HistoryViewModel>().checkIfIsSelected(index)
                                ? context.read<HistoryViewModel>().removeSelectedIndex(index)
                                : context.read<HistoryViewModel>().selectItem(index);
                          })
                        },
